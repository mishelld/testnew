 ...
 */
public class MyDirectedWeightedGraphAlgorithmsImpl implements DirectedWeightedGraphAlgorithms {
    ...
    public DirectedWeightedGraph copy() {
        DirectedWeightedGraph copy1 = new DirectedWeightedGraphImpl();
        Iterator iterNode = this.graph.nodeIter();
        for (Iterator<NodeData> it2 = iterNode; it2.hasNext(); ) {
            NodeData node = (NodeData) iterNode.next();
            NodeData newNode = new NodeDataImpl(node);
            copy1.addNode(newNode);
        }
        Iterator iterEdge = graph.edgeIter();
        for (Iterator<EdgeData> it3 = iterNode; it3.hasNext(); ) {
            EdgeData edge = (EdgeData) iterEdge.next();
            copy1.connect(edge.getSrc(), edge.getDest(), edge.getWeight());
        }
