...
public class graphG3 extends JFrame implements ActionListener {
    ...
    graphG3(DirectedWeightedGraph ans){
        super();
        graphAlgo = ans;
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (s.equals("get node")) {
            new GetNode(Ex2.getGrapg("data/G3.json"));
        }
        else if(s.equals("get edge")){
            new ShortestPathDist(Ex2.getGrapgAlgo("data/G3.json"));

        } else if(s.equals("size of nodes")){
            String message = "The Size Of The Nodes In The Graph is: " + Ex2.getGrapg("data/G3.json").nodeSize();
            JOptionPane.showMessageDialog(new JFrame(), message, "Size Of Nodes", JOptionPane.DEFAULT_OPTION);


        } else if(s.equals("size of edges")) {
            String message = "The Size Of The Nodes In The Graph is: " + Ex2.getGrapg("data/G3.json").edgeSize();
            JOptionPane.showMessageDialog(new JFrame(), message, "Size Of Nodes", JOptionPane.DEFAULT_OPTION);
        }
        else if(s.equals("is graph connected")){
            String message;

            if(Ex2.getGrapgAlgo("data/G3.json").isConnected()) {
            ...
            new Load(Ex2.getGrapgAlgo("data/G3.json"), panel);
        }
        else if(s.equals("remove edge")){
            new RemoveEdge(Ex2.getGrapg("data/G3.json"), panel);


        }
        else if(s.equals("remove node")){
            new RemoveNode(Ex2.getGrapg("data/G3.json"), panel);

        }
        else if(s.equals("save")){
            new Save(Ex2.getGrapgAlgo("data/G3.json"));
        }
        else if(s.equals("shortest path")){
            new ShortestPath(Ex2.getGrapgAlgo("data/G3.json"));
            ...
            new TSP(Ex2.getGrapgAlgo("data/G3.json"));
